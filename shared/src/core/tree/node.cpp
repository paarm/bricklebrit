...
ContentType getContentTypeFromString(const string &rContentTypeString) {
	...
	} else if (rContentTypeString=="Container") {
...
Node& Node::addChildNode(ContentType rContentType) {
	mNodes.emplace_back(rContentType);
	Node &rv=mNodes.back();
	rv.mParent=this;
	return rv;
}
		...
		if (rJSONValueParent->IsObject()) {
			...
			if (rChildNodes && rChildNodes->IsArray()) {
				...
				for (auto *rJSONValue : rJSONArray) {
					...
					Node &rChildNode=addChildNode(getContentTypeFromString(contentType));
